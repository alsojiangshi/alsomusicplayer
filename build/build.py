...
产物 (×2 平台):
    dist/linux/MusicPlayer-cli-linux       — CLI/TUI (终端)
    dist/linux/MusicPlayer-gui-linux       — GUI (图形界面)
    dist/linux/MusicPlayer-full-linux      — CLI + GUI 合一版
    dist/windows/MusicPlayer-cli-windows.exe
    dist/windows/MusicPlayer-gui-windows.exe
    dist/windows/MusicPlayer-full-windows.exe
"""
...
CLI_HIDDEN = [
    ...
    "rich", "markdown_it",
    "pygame", "pygame.mixer", "pygame.event",
]
]
...
def build_variant(variant_key: str, platform: str) -> bool:
    ...
    if icons_dir.exists():
        datas.extend(["--add-data", f"{icons_dir}{os.pathsep}resources/icons"])
    if fonts_dir.exists():
        datas.extend(["--add-data", f"{fonts_dir}{os.pathsep}resources/fonts"])
    if tcss.exists():
        datas.extend(["--add-data", f"{tcss}{os.pathsep}src/cli"])
