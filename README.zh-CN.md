...
本地音频保留在原始位置，不会复制到应用存储目录。项目软件代码采用 [MIT License](./LICENSE) 授权，但须遵守下文所述的排除条款。
...
项目的软件源代码、构建脚本、配置、测试及编译后的代码采用 MIT License；项目文档及其它非代码材料除非另有明确声明，否则不在 MIT 授权范围内。千石抚子应用图标以及 `packages/gui/src-tauri/icons/` 下所有生成或嵌入的变体均明确排除在 MIT 授权范围之外。角色、物语系列原作、源美术作品、名称及相关权利归各自权利人所有；本项目与相关权利人不存在隶属、赞助或背书关系。项目不授予复用或再分发这些图标资产的许可。第三方依赖以及用户导入或从在线服务取得的内容仍分别受其自身权利与许可条款约束。具有约束力的授权范围和完整声明以 [LICENSE](./LICENSE) 为准。
